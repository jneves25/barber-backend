...
export class RevenueService {
	...
	private getPeriodDates(period: string, year: number, month?: number, customStartDate?: Date, customEndDate?: Date): PeriodDates {
	...
	async getMonthlyRevenue(companyId: number, year: number, period?: string, customStartDate?: Date, customEndDate?: Date) {
	...
	async getBarberRevenue(companyId: number, period: string, year: number, month?: number, customStartDate?: Date, customEndDate?: Date) {
		...
		const companyOwners = await prisma.company.findMany({
			where: {
				id: companyId,
				deletedAt: null
			},
	...
	async getBarberMonthlyRevenue(companyId: number, year: number, customStartDate?: Date, customEndDate?: Date) {
		const months = [
			'Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun',
			'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'
		];
		...
		for (let month = 0; month < 12; month++) {
			...
			const monthData: any = { name: months[month] };
	...
	async getPaymentMethodRevenue(
		...
		return paymentMethods.map(method => ({
			...method,
			value: (totalRevenue * method.percentage) / 100
		}));
	}
	...
	async getYearlyComparison(companyId: number, currentYear: number, customStartDate?: Date, customEndDate?: Date) {
		const months = [
			'Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun',
			'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'
		];
