...
export class StatisticsController {
	...
	static async getBarberCommissions(req: Request, res: Response): Promise<void> {
		try {
			...
			const formattedCommissions = await Promise.all(filteredCommissions.map(async (item: any) => {
				...
				const commissionPercent = commissionConfig?.commissionValue || 20; // Padrão 20%
	...
	static async getProjections(req: Request, res: Response): Promise<void> {
		try {
			...
			const barberProjections = await Promise.all(commissions.map(async (barber: any) => {
				...
				const pendingValue = barberPendingApps.reduce((sum: number, app: any) => sum + app.value, 0);
				...
				if (commissionConfig?.commissionValue !== undefined && commissionConfig?.commissionValue !== null) {
